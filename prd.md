...
For the first slice, the user needs `myn` to prove the Stdio Lease contract locally and on Unix-like Personal Server environments without implementing hibernation, SSH detection, tmux detection, systemd scheduling, or non-interactive command leases.

## Solution

Add stdio-only Idle Lease support to the existing Go `myn` CLI.

The user can run an interactive command through:

```sh
myn run --stdio --idle-after 30m -- codex
myn run --stdio --idle-after 30m -- claude
myn run --stdio -- bash
```
...
```sh
myn idle status
myn idle status --json
```

`myn idle status` is read-only in this slice. It reports active, idle, and stale leases without pruning files.
...
13. As a Personal Server User, I want the wrapped command's exit code to pass through, so that using `myn run --stdio` does not change command semantics.
...
16. As a Personal Server User, I want local development to use `MYN_LEASE_DIR`, so that I can test leases without needing `/run` permissions.
...
22. As a Personal Server User, I want `myn idle status` to summarize lease counts, so that I can quickly see whether the server would be considered busy.
23. As a Personal Server User, I want `myn idle status` to show each lease state, so that I can diagnose active, idle, and stale sessions.
...
25. As an automation author, I want `myn idle status --json`, so that tests and later agents can consume lease state without scraping human text.
26. As an automation author, I want `myn idle status` to exit zero when it successfully reports idle or stale leases, so that state is data rather than command failure.
...
33. As a developer of `myn`, I want plain `myn run -- <command>` to fail clearly for now, so that users do not think non-stdio command leases are already implemented.
34. As a developer of `myn`, I want unsupported platforms to fail clearly, so that there is no silent degradation from PTY behavior to pipe behavior.
35. As a developer of `myn`, I want the lease contract to be testable without running a full Personal Server, so that this slice can be developed locally.
36. As a developer of `myn`, I want the lease state evaluator isolated from PTY plumbing, so that active/idle/stale behavior can be unit tested thoroughly.
37. As a developer of `myn`, I want the PTY runner isolated from status rendering, so that terminal proxy behavior and lease classification do not become entangled.
38. As a developer of `myn`, I want the JSON status contract tested, so that future hibernation and idle-agent work can rely on it.

## Implementation Decisions

- The implementation adds `myn run` and `myn idle status` command groups to the existing Cobra CLI.
- The only supported `myn run` mode in this slice is `--stdio`.
- Running `myn run` without `--stdio` should fail clearly with a message that non-stdio command leases are not implemented yet.
- `myn run --stdio` accepts `--idle-after`, using Go-style duration strings.
- The default Stdio Lease idle window is 30 minutes.
- Zero or negative idle windows are invalid.
- `myn run --stdio` requires terminal-backed stdin and stdout.
- `myn run --stdio` uses a PTY, not plain pipes, in accordance with the PTY-backed Stdio Leases ADR.
...
- `rootPid` is the only process-liveness anchor used by `myn idle status` in this slice.
- `processGroup` may be recorded for future command-lease work, but it does not define stdio liveness yet.
- The runtime lease directory defaults to `/run/myn/idle/leases`.
- `MYN_LEASE_DIR` overrides the runtime lease directory for local development and tests.
...
- `myn idle status` is read-only and does not remove stale lease files.
- `myn idle status --json` should return a stable object with lease directory, current time, counts by state, and per-lease details.
- `myn idle status` exits zero when it successfully reports state, even when idle or stale leases exist.
...
- Tests should use temporary lease directories through `MYN_LEASE_DIR` or injected dependencies.
...
- Manual inhibitors such as `myn idle inhibit`, `myn idle disable`, or `myn idle mark-active`.
- Non-stdio command leases for `myn run -- <command>`.
...
- Pruning stale leases from `myn idle status`.
...
- The PTY-backed Stdio Leases ADR records why `myn run --stdio` must use terminal semantics instead of plain pipes.
